...
- Plugins can provide custom tools (with Zod schemas) and hooks, but cannot directly register agents/skills/commands — they must create the files on disk
...
|----------|-----------|---------|
| Build fresh assets, don't port existing | User doesn't use most existing Claude Code assets; curated > comprehensive | — Pending |
| Creation tooling first, then curated assets | Infrastructure enables future growth; assets without tooling is a dead end | — Pending |
| Single npm package distribution | Simplest install path: add one line to opencode.json | — Pending |
| Commands as the creation UX | OpenCode commands run in-session, closest to Claude Code's slash command experience | — Pending |
